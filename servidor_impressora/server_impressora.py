...
def formatar_comanda_escpos(data):
    ...
    cmd.extend(linha_sep('=').encode('cp850', errors='ignore') + b'\n')
    cmd.extend(FEED(1))
    mensagem_rodape = data.get('mensagem_rodape', 'Obrigado pela preferencia!')
    cmd.extend(BOLD_ON)
    cmd.extend(f'{mensagem_rodape}\n'.encode('cp850', errors='ignore'))
    cmd.extend(BOLD_OFF)
    cmd.extend('XRA AutoPay\n'.encode('cp850', errors='ignore'))
    cmd.extend(FEED(4))
...
if __name__ == '__main__':
    print('=' * 60)
    print('  SERVIDOR DE IMPRESSAO + TEF - XRA AutoPay')
